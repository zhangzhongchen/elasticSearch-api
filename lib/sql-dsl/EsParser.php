    ...
    private function whereext($arr, $i)
    {
        ...
        switch ($lowerstr) {
            ...
            case 'like':
                ...
                $tmp_la_str = str_replace('"', '', $arr[$i + 1]['base_expr']);
                $tmp_la_str = str_replace("'", "", $tmp_la_str);
                $namevalue = str_replace("%", "", $tmp_la_str);

                if (is_array(Config::getIndex($this->index_es, 'searchBoostList'))) {
                    $searchList = Config::getIndex($this->index_es, 'searchBoostList');
                    isset($searchList[$termk]) || $searchList[$termk] = ['boost' => 1];
                    $i = 0;
                    foreach ($searchList as $k2 => $Boost) {
                        $where = 'should';
                        ($i == 0) && $where = 'must';
                        $data[$where][]['match'][$k2] = ['query' => $namevalue, 'boost' => $Boost['boost']];
                        $i++;
                    }
                } else {
                    $data['must'][] = ['match' => [$termk => ['query' => $namevalue]]];
                }
                $likeArray = $data;
                ...
                if (isset($arr[$i + 2]['base_expr']) && strtolower($arr[$i + 2]['base_expr']) == 'or' || isset($arr[$i - 2]['base_expr']) && strtolower($arr[$i - 2]['base_expr']) == 'or') {
                    ...
                    if (!is_numeric($arr[$i + 1]['base_expr']) && $this->version_es == '8.x') {
                        $this->Builderarr['query']['bool']['filter'][$this->count_tmp_filter]['bool'] = $likeArray;
                    } else {
                        $this->Builderarr['query']['bool']['filter'][$this->count_tmp_filter]['bool'] = $likeArray;
                    }
                } else {
                    ...
                    if (!is_numeric($arr[$i + 1]['base_expr']) && $this->version_es == '8.x') {
                        $this->Builderarr['query']['filter'][$this->count_tmp_filter]['must'][$this->count_tmp]['bool'] = $likeArray;
                    } else {
                        $this->Builderarr['query']['bool']['filter'][$this->count_tmp_filter]['bool'] = $likeArray;
                    }
                }
